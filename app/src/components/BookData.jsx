...
// export default function BookData({ bookId: propBookId }) {
...
//       try {
//         let id = bookId;
//         // If no bookId provided, fetch list and pick one randomly
//         if (!id) {
//           const listRes = await fetch(API.local.books);
//           if (!listRes.ok) throw new Error(`HTTP ${listRes.status}`);
//           const list = await listRes.json();
//           if (!list.length) throw new Error('No books available');
...
export default function BookData({ bookId }) {
	const [book, setBook] = useState(null);
	const [imageError, setImageError] = useState(false);
	const [loading, setLoading] = useState(true);
	const [error, setError] = useState(null);
