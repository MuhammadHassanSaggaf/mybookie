...
export default function Sidebar({ onBookClick , filter }) {
		...
		{(!filter || filter === "all") && (
			<section className="mb-6 border-b border-white/10 pb-4">
				...
				>
					All Books
					<ChevronRightIcon
						className={`h-4 w-4 transition-transform duration-200 ${open.all ? "rotate-90" : ""}`}
					/>
				</h2>
		...
		{(!filter || filter === "favourites") && (
			<section className="mb-6 border-b border-white/10 pb-4">
				...
				>
					Favourites
					<ChevronRightIcon
						className={`h-4 w-4 transition-transform duration-200 ${open.favourites ? "rotate-90" : ""}`}
					/>
				</h2>
		...
		{(!filter || filter === "borrowed") && (
			<section className="border-b border-white/10 pb-4">
				...
				>
					Borrowed
					<ChevronRightIcon
						className={`h-4 w-4 transition-transform duration-200 ${open.borrowed ? "rotate-90" : ""}`}
					/>
				</h2>
